...
\label{eq:actual-boris1}
  ...
  \end{align}
\end{subequations}
This is not yet the Boris push. In order to be able to implement this scheme in an actual code, we need a way to separate \(\vb{v_{n+\frac{1}{2}}}\) and \(\vb{v_{n-\frac{1}{2}}}\) from the second equatin. Boris came up with a method to separate the action of the electric and magnetic fields by employing the following three equations
...
\label{eq:actual-boris2}
  ...
  \end{align}
\end{subequations}
The full classical Boris push is described by~\cref{eq:actual-boris11,eq:actual-boris2} put together. Note that we basically work only with mid-step velocities. But we can always recover the actual velocities from~\cref{eq:borisp2} like this
...
We introduced~\cref{eq:actual-boris2} out of nowhere, but it is quite straightforward to see that by eliminating \(v^-\) and \(v^+\) we simply recover~\cref{eq:actual-boris12}. Besides this, there is a geometrical Interpretation to the break up described by~\cref{eq:actual-boris2}. \(v^-\) and \(v^+\) enclose the drift motion due to electric field, \cref{eq:actual-boris2} described a rotation under the effect of a constant magnetic field. For a more in depth description with imagise to help visualization I recommend the Master Thesis of~\cite{micluta-campeanuLaserWakefieldAcceleration2019}.

\subsubsection{Numerical Propeties (Symplecticity and Volume Conservation Theory)}
...
where \(J = \begin{bmatrix}
  0 & I \\
  -I & 0
\end{bmatrix}\) and \(I\) is the \(n\)-dimensional unitary matrix.

% offer proof

This is the complete set of mathematical definitions and nomenclature that develop the concept of symplectic maps. But a question still remains: what is a symplectic map \textit{really}? Well, it is a generalization of area-preservance (the image of a subset of our manifold through an area-preserving map has the same volume as the subset itself). In practice it is useful to use the following result in order to decide if this property holds: \textbf{a linear map is area-preserving if its associated matrix has the determinant equal to 1}. The proof is omitted since it can be easily found on many places across the internet. Now, looking at phase space, if a linear map is symplectic then, informally, the sum of areas projected on the planes \((\vb{q_i},\vb{p_i}),\;i = \overline{1,n}\) is conserved~(\cite{weissteinSymplecticMap}). In fact, area-preservance is a necessary, but insufficient condition for symplecticity.

We still have to make one ore step in order to study the properties of the Boris push. The problem is that the map that describes this scheme is not linear, so we can not directly use what we presented so far. We simply have to define simplecticity in a larger sense.
